...
function main()
	...
	async_http_request('http://f0294346.xsph.ru/kill-state-version.php', nil,
	function(version)
		if version ~= nil and version > thisScript().version then
			...
			downloadUrlToFile('http://f0294346.xsph.ru/update-kill-state.php', thisScript().path,
	...
	if not doesFileExist('moonloader/config/icons.png') then
		while true do wait(0)
			if connect == false then
				...
				sampAddChatMessage('{A800FF}[Kill State by Eenz Hatte]{ffffff}Перейди по ссылке "http://f0294346.xsph.ru/icons.php", скачай картинку,', color)
				...
				sampRegisterChatCommand('copylink', function()
					setClipboardText('http://f0294346.xsph.ru/icons.php')
				end)
				...
				wait(-1)
			elseif connect then
				downloadUrlToFile('http://f0294346.xsph.ru/icons.php', getWorkingDirectory()..'\\config\\icons.png',
				function(id, status, p1, p2)
					if status == dls.STATUS_ENDDOWNLOADDATA then
